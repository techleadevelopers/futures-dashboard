...
import { computeAllCandleEdges, type CandleInterval } from "../lib/candleEdge";
...
router.patch("/bot/config/override", (req: Request, res: Response) => {
  ...
  const allowed = [
    ...
    "btcRegimeRequired", "btcRegimeThresholdPct", "allowedSymbols", "hourBlacklist",
...
router.post("/bot/order/bulk", async (req: Request, res: Response) => {
  ...
  req.log.info({ count: orders.length, rps, activeMode }, "bulk execution started");

  for (let i = 0; i < orders.length; i++) {
    await bucket.consume(); // respect rate limit
    const result = await executeSingleOrder(orders[i], i, creds, config);
    results.push(result);
  }
