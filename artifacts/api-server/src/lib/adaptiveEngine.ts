...
const EWMA_FAST = 0.20; // rapid adaptation: per-trade win rate
...
export class AdaptiveEngine {
  ...
  recordOutcome(outcome: TradeOutcome): void {
    ...
    const win = outcome.realizedPnl > 0;

    // Update global EWMA
    this.globalEwmaWinRate = ewma(this.globalEwmaWinRate, win ? 1 : 0, EWMA_FAST);
    this.globalEwmaEv = ewma(this.globalEwmaEv, outcome.realizedPnl, EWMA_SLOW);
  ...
  gateRecommendation(): AdaptiveGateRecommendation {
    ...
    const positiveEvClusters = Array.from(this.clusterProfiles.values())
      .filter((p) => p.ev > 0 && p.samples >= MIN_SAMPLES_FOR_GATE)
      .map((p) => p.ev)
      .sort((a, b) => a - b);
  ...
  private updateClusterProfile(outcome: TradeOutcome): void {
    ...
    if (!existing) {
      ...
      const avgWin = win ? outcome.realizedPnl : 0;
      const avgLoss = !win ? outcome.realizedPnl : 0;
      ...
      const realizedCapture = outcome.exitReason === "TP" && outcome.expectedTpProfit > 0
        ? clamp(outcome.realizedPnl / outcome.expectedTpProfit, 0, 1.5)
        : 0.5;
      ...
      this.clusterProfiles.set(keyStr, {
        ...
        avgWin,
        avgLoss,
        ev: outcome.realizedPnl, // single sample
        profitFactor,
        totalPnl: outcome.realizedPnl,
    ...
    const avgWin = win
      ? existing.avgWin + (outcome.realizedPnl - existing.avgWin) / Math.max(wins, 1)
      : existing.avgWin;
    const avgLoss = !win
      ? existing.avgLoss + (outcome.realizedPnl - existing.avgLoss) / Math.max(losses, 1)
      : existing.avgLoss;
    ...
    const realizedCapture = ewma(existing.realizedCapture, tradeCapture, EWMA_FAST);

    // EWMA win rate and EV
    const ewmaWinRate = ewma(existing.ewmaWinRate, win ? 1 : 0, EWMA_FAST);
    ...
    this.clusterProfiles.set(keyStr, {
      ...
      ev,
      profitFactor,
      totalPnl: existing.totalPnl + outcome.realizedPnl,
