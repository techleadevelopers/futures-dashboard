...
function computeScores(
  ...
  const continuationLong = clamp01(emaBullStrength * 0.45 + rsiLong * 0.35 + volBoost * 0.20);
  const continuationShort = clamp01(emaBearStrength * 0.45 + rsiShort * 0.35 + volBoost * 0.20);

  // Scalp contrarian: after a fast 2-4% candle move, prefer the short rebound.
  ...
  const contrarianLong = clamp01(dumpStrength * 0.70 + oversoldBoost * 0.20 + volBoost * 0.10);
  const contrarianShort = clamp01(pumpStrength * 0.70 + overboughtBoost * 0.20 + volBoost * 0.10);
  const reversalScore = clamp01(Math.max(dumpStrength, pumpStrength) * 0.78 + volBoost * 0.22);
